...
<!--          {{ scope.$index }}-->
      ...
      <el-table-column label="该类目是否为父类目" >
        <template slot-scope="scope">
...
<script>
  import { fetchList, fetchPv, createContentCategory, updateContentCategory,deleteContentCategory } from '@/api/content/contentcategory/contentcategory'
    ...
    methods: {
      ...
      createData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            createContentCategory(this.temp).then(() => {
      ...
      updateData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            updateContentCategory(tempData).then(() => {
      ...
      handleDelete(row) {
        this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
          ...
          deleteContentCategory(row.id).then(response => {
            this.getList()
            this.$message({
              type: 'success',
              message: '删除成功!'
