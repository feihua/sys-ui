...
<!--          {{ scope.$index }}-->
      ...
      <el-table-column label="商品类目ID" width="60">
        <template slot-scope="scope">
          {{ scope.row.itemCatId }}
        </template>
      </el-table-column>
      <el-table-column label="参数数据">
        <template slot-scope="scope">
          <span>{{ scope.row.paramData }}</span>
        </template>
      </el-table-column>
...
<script>
  import { fetchList, fetchPv, createItemParam, updateItemParam,deleteItemParam } from '@/api/item/itemparam'
    ...
    methods: {
      ...
      createData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            createItemParam(this.temp).then(() => {
      ...
      handleDelete(row) {
        this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
          ...
          deleteItemParam(row.id).then(response => {
            this.getList()
            this.$message({
              type: 'success',
              message: '删除成功!'
