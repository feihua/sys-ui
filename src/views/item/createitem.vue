  ...
  <div class="app-container" style="margin-left: 50px">
    <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px;">
      <el-form-item label="商品类目" prop="name">
        <el-button type="primary" @click="handleCreate">选择类目</el-button>
        {{catname}}
      </el-form-item>
      <el-form-item label="商品标题" prop="age">
        <el-input v-model="temp.title"/>
      </el-form-item>
      <el-form-item label="商品卖点" prop="phone">
        <el-input v-model="temp.sellPoint" :autosize="{ minRows: 3, maxRows: 5}" type="textarea"/>
      </el-form-item>
      <el-form-item label="商品价格" prop="email">
        <el-input v-model="temp.price"/>
      </el-form-item>
      <el-form-item label="库存数量" prop="email">
        <el-input v-model="temp.num"/>
      </el-form-item>
      <el-form-item label="条形码" prop="email">
        <el-input v-model="temp.barcode"/>
      </el-form-item>
      <el-form-item label="商品状态" prop="email">
      ...
      <el-form-item label="商品图片" prop="email">
        <el-upload
          class="upload-demo"
    ...
    <el-dialog title="商品类目" :visible.sync="dialogFormVisible">
      <el-tree
        :data="list"
        show-checkbox
        node-key="id"
...
<script>
  ...
  import { fetchList, fetchItemCat } from '@/api/item/itemcat'
  import { createItem, updateItem, deleteItem } from '@/api/item/item'
  ...
  export default {
    ...
    methods: {
        ...
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            createItem(this.temp).then(() => {
              ...
              this.$router.push({ path: this.redirect || '/item/list' })
            })
          }
        })
      },
      ...
      getCheckedKeys() {
        ...
        fetchItemCat(this.catid[0]).then(response => {
